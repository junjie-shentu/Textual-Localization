...
def main(args):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("textual-localization_multi_soft_guidance", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                zero_area_1 = (attention_maps_1 == 0).squeeze(1) # a tensor of boolean values with the elememt being True if the corresponding element in attention_maps_1 is 0
                zero_area_2 = (attention_maps_2 == 0).squeeze(1) # a tensor of boolean values with the elememt being True if the corresponding element in attention_maps_2 is 0
                ...
                if args.with_prior_preservation:
                    ...
                    attention_loss_1 = attention_loss_1 * zero_area_1
                    attention_loss_1 = torch.mean(attention_loss_1)
                    attention_loss_2 = torch.nn.MSELoss(reduction='none')(cross_attention_maps_2, attention_maps_2.squeeze(1))
                    attention_loss_2 = attention_loss_2 * zero_area_2
                    attention_loss_2 = torch.mean(attention_loss_2)
